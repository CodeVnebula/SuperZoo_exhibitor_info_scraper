...
with open('Exhibitors_temp.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    for i in range(len(link_ids)):      # Iterating over each of found ID's !(Same amount as count)                                                        # Adding Saved ID's 6 digit
        ...
        ##
        # Some companies doesn't have description or brands section they have description but not brands
        # Mostly if there is brand section then there must be description too
        # Description is always before Brands section (if have) So by finding Brands we can find description
        # One thing is that if there is description but not brands then we're loosing description too (Less likely but possible)        
        ...
        print(f'{((i + 1) / len(link_ids)) * 100:.3f}% Complete...')        # (:.3f) for saving only 3 numbers after comma
